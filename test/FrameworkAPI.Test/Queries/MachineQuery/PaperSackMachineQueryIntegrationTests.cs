...
public class PaperSackMachineQueryIntegrationTests
{
    ...
    [Fact]
    public async Task GetPaperSackMachine_Should_Return_Speed()
    {
        // Arrange
        var executor = await InitializeExecutor();

        InitializePaperSackTuberMachineMock();

        var snapshotMetaDto = new SnapshotMetaDto(
            MachineId,
            "fakeHash",
            DateTime.MinValue,
            [new SnapshotColumnUnitDto(SnapshotColumnIds.PaperSackSpeed, "unit.itemsPerMinute")]);
        var snapshotDto = new SnapshotDto(
            [new SnapshotColumnValueDto(SnapshotColumnIds.PaperSackSpeed, 77.7)],
            DateTime.UtcNow);
    ...
    [Fact]
    public async Task GetPaperSackMachine_Should_Return_Trend_Of_Speed()
    {
        // Arrange
        var executor = await InitializeExecutor();
        ...
        for (var i = 0; i < (int)Constants.MachineTrend.TrendTimeSpan.TotalMinutes; i++)
        {
            snapshotDtos.Add(new SnapshotDto(
                [new SnapshotColumnValueDto(SnapshotColumnIds.PaperSackSpeed, i == 0 ? null : i * 1.5)],
    ...
    private void InitializePaperSackTuberMachineMock()
    {
        var machine = MachineMock.GeneratePaperSackTuber(MachineId);
