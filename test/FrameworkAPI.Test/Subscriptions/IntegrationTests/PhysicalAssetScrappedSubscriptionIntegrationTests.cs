...
public class PhysicalAssetScrappedSubscriptionIntegrationTests : ReactiveTest
{
    ...
    public PhysicalAssetScrappedSubscriptionIntegrationTests()
    {
    ...
    public async Task WhenPhysicalAssetScrapped_Should_Emit_Published_Values_When_PhysicalAsset_Created(
        PhysicalAssetType? physicalAssetTypeFilter)
    {
        ...
        _capturedSubscribeToPhysicalAssetScrappedEventCallback.Should().NotBeNull();
    ...
    private static IQueryRequest GetSubscriptionQuery(PhysicalAssetType? physicalAssetTypeFilter)
    {
        return QueryRequestBuilder
            .New()
            .SetQuery(
                $"subscription {{ physicalAssetScrapped(physicalAssetTypeFilter: {physicalAssetTypeFilter?.GetEnumMemberValue().ToUpper() ?? "null"}) {{ physicalAssetId }} }}")
    ...
    {
        var objectResult = result.Data!["physicalAssetScrapped"] as IReadOnlyDictionary<string, object>;
