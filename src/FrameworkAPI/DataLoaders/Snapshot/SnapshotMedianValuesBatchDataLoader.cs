...
public class SnapshotMedianValuesBatchDataLoader : BatchDataLoader<SnapshotValueRequestKey, DataResult<double?>>
{
    private readonly IMachineSnapshotHttpClient _machineSnapshotHttpClient;

    public SnapshotMedianValuesBatchDataLoader(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var snapshotClientResponseTasks = batches
            .Select(async group =>
            {
                ...
                var response = await _machineSnapshotHttpClient.GetMedianValues(
                    machineId,
