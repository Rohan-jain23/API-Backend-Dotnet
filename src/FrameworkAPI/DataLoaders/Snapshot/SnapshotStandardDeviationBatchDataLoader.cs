...
public class SnapshotStandardDeviationBatchDataLoader : BatchDataLoader<SnapshotValueRequestKey, DataResult<double?>>
{
    private readonly IMachineSnapshotHttpClient _machineSnapshotHttpClient;

    public SnapshotStandardDeviationBatchDataLoader(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var snapshotClientResponseTasks = batches
            .Select(async group =>
            {
                ...
                var response = await _machineSnapshotHttpClient.GetStandardDeviations(
                    machineId,
