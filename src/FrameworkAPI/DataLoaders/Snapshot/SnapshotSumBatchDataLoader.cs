...
public class SnapshotSumBatchDataLoader : BatchDataLoader<SnapshotValueRequestKey, DataResult<double?>>
{
    private readonly IMachineSnapshotHttpClient _machineSnapshotHttpClient;

    public SnapshotSumBatchDataLoader(
    ...
    protected override async Task<IReadOnlyDictionary<SnapshotValueRequestKey,
        DataResult<double?>>> LoadBatchAsync(IReadOnlyList<SnapshotValueRequestKey> keys,
        CancellationToken cancellationToken)
    {
        ...
        var snapshotClientResponseTasks = batches
            .Select(async batch =>
            {
                var response = await _machineSnapshotHttpClient.GetSumValues(
                    batch.MachineId,
